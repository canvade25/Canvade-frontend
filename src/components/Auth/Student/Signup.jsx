...
export default function Signup({ switchToLogin, onSignupSuccess, switchToSelect }) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      onSignupSuccess?.();
      setParentFormData({ email, password, displayName: name });
  ...
  if (showOnboardingSteps) {
    return (
      <OnboardingSteps
        back={() => setShowOnboardingSteps(false)}
      ...
      <div className="space-y-3">
        <div className="flex flex-col items-center space-y-2">
            <label htmlFor="profileImage" className="cursor-pointer">
              ...
              <span className="text-sm text-gray-600 mt-2">Upload Profile Image</span>
            </label>
            <input
              id="profileImage"
              type="file"
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-24 shrink-0 items-center justify-center rounded border border-gray-200 bg-gray-50 ">
